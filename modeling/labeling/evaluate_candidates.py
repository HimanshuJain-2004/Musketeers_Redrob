...
async def main():
    ...
    with open(os.path.join(WORKSPACE_ROOT, 'modeling', 'prompts', 'system_prompt.txt'), 'r') as f:
        sys_prompt = f.read()
    with open(os.path.join(WORKSPACE_ROOT, 'modeling', 'prompts', 'candidate_prompt.txt'), 'r') as f:
        cand_prompt = f.read()
    with open(os.path.join(WORKSPACE_ROOT, 'modeling', 'prompts', 'job_description.txt'), 'r', encoding='utf-8') as f:
        jd_text = f.read()
    with open(os.path.join(WORKSPACE_ROOT, 'feature_engineering', 'artifacts', 'jd_requirements.json'), 'r') as f:
        jd_json = json.load(f)
